	          ...
	          <div class="col-md-12">
	            <div class="card card-default">
	              <div class="card-header">
	                <h3 class="card-title">{{ $card_title }} of students of 
	                	<a href="javascript:void(0);">{{ $center->name }}</a> Class
	                	<a href="javascript:void(0);">{{ $class->name }}</a> Subject
	                	<a href="javascript:void(0);">{{ $subject->name }}</a> for 
	                	<a href="javascript:void(0);">{{ $semester->title }}</a> and
	            ...
	            @endif
	            <form id="quickForm" method="post" action="{{ route('exams.storedatabystudents') }}" class="form-horizontal" enctype="multipart/form-data">
                	{{ csrf_field() }}
	              ...
	              <div class="card-body">
	              	<div class="row">
		              	<div class="col-lg-1">
		                    <label class="text-align-center" for="labelInputStudent">Roll No<i class="fa fa-star-of-life required-label"></i></label>
			           </div>
			           <div class="col-lg-2">
		                    <label class="text-align-center" for="labelInputStudent">Student<i class="fa fa-star-of-life required-label"></i></label>
			           </div>
			           <div class="col-lg-1">
		                ...
		                <div class="col-lg-2">
		                    ...
		                    <p style="margin:0px; padding:0px; margin-top:32px;"></p>
		                    <div class="form-group clearfix">
			                	<div style="width:10%;" class="centerer">
			                      <div class="icheck-success d-inline">
			                        <input type="checkbox" id="check_box_sheet_no" value="0">
			                        <label for="check_box_sheet_no" data-toggle="tooltip" data-placement="top" title="Copy value of first element in all elements shown below.">
			                        </label>
			                      </div>
			                    </div>
		                    </div>
		               ...
		               <div class="col-lg-1">
		                    ...
		                    <div class="form-group clearfix">
			                	<div class="centerer">
			                      <div class="icheck-success d-inline">
			                        <input type="checkbox" id="check_box_theory_max_marks" value="0">
			                        <label for="check_box_theory_max_marks" data-toggle="tooltip" data-placement="top" title="Copy value of first element in all elements shown below.">
			                        </label>
			                      </div>
			                    </div>
		                    </div>
		                ...
		                <div class="col-lg-1">
		                    ...
		                    <div class="form-group clearfix">
			                	<div class="centerer">
			                      <div class="icheck-success d-inline">
			                        <input type="checkbox" id="check_box_practical_max_marks" value="0">
			                        <label for="check_box_practical_max_marks" data-toggle="tooltip" data-placement="top" title="Copy value of first element in all elements shown below.">
			                        </label>
			                      </div>
			                    </div>
		                    </div>
	              	...
	              	<div id="field_wrapper">
			            @foreach($students as $student)
			            	<?php $se = App\Models\StudentsExam::getstudentsubjectdetails($student->id, $session->id, $semester->id, $class->id, $center->id, $subject->id); ?>
			              		<div class="dynamic_wrapper">
				              		<div class="row">
						              	<div class="col-lg-1">
							              	<div class="form-group">
							                    <input readonly="true" type="number" name="roll_no[]" class="form-control" placeholder="Roll No" value="{{ $student->id }}">
							               </div>
							           </div>
							           <div class="col-lg-2">
							              	<div class="form-group">
							                    <select class="custom-select rounded-0 is-valid" name="student_id[]" aria-invalid="false">
							                    	<option data-haspractical="{{ $subject->has_practical }}" value="{{ $student->id }}">{{ $student->name }}</option>
							                    </select>
							               </div>
							           </div>
							                ...
							                <div class="form-group clearfix">
							                	<div class="centerer">
							                      <div class="icheck-danger d-inline">
							                        <input {{ is_null($se) ? '':$se->is_absent == 1 ? 'checked':'' }} type="checkbox" name="is_absent[]" id="check_box_{{ $student->id }}" value="{{ is_null($se) ? '0':$se->is_absent }}">
							                        <input type="hidden" name="is_absent_val[]" value="{{ is_null($se) ? '0':$se->is_absent }}">
							                        <label for="check_box_{{ $student->id }}">
							                        </label>
							                      </div>
							                    </div>
						                    </div>
							           ...
							           <div class="col-lg-2 sheet_no">
						                ...
						                <div class="col-lg-1 practical_marks">
							                <div class="form-group">
							                    <input {{ $subject->has_practical == 1 ? '':'readonly' }} type="number" name="practical_max_marks[]" class="form-control" placeholder="Practical Max Marks" value="{{ $subject->has_practical == 1 ? is_null($se) ? '0':$se->practical_max_marks:'0' }}">
						                   </div>
						                </div>
						                <div class="col-lg-1 practical_marks">
							                <div class="form-group">
							                    <input {{ is_null($se) ? '':$se->is_absent == 1 ? 'readonly':'' }} {{ $subject->has_practical == 1 ? '':'readonly' }} type="number" name="practical_obt_marks[]" class="form-control" placeholder="Practical Obt Marks" value="{{ $subject->has_practical == 1 ? is_null($se) ? '0':$se->practical_obt_marks:'0' }}">
						                   </div>
						                </div>
...
<script>
	...
	$(document).on('change', '#check_box_theory_max_marks', function(){
		var checked = $(this).is(':checked');

		if(checked){
			var i=0;
			var first_value = '';
			$('input[name="theory_max_marks[]"]').each(function(){
				if(i==0){
			...
			$('input[name="theory_max_marks[]"]').each(function(){
				...
				var prc_max_marks = $(this).closest('.dynamic_wrapper').find('input[name="practical_max_marks[]"]').val();
				var the_max_marks = $(this).val();
				var t_max_marks = parseInt(prc_max_marks) + parseInt(the_max_marks);
				
				$(this).closest('.dynamic_wrapper').find('input[name="total_max_marks[]"]').val(t_max_marks);
			});
		}
	});

	$(document).on('change', '#check_box_practical_max_marks', function(){
		var checked = $(this).is(':checked');

		if(checked){
			var i=0;
			var first_value = '';
			$('input[name="practical_max_marks[]"]').each(function(){
				if(i==0){
					first_value = $(this).val();
					return false;
				}
			...
			$('input[name="practical_max_marks[]"]').each(function(){
				...
				var prc_max_marks = $(this).val();
				var the_max_marks = $(this).closest('.dynamic_wrapper').find('input[name="theory_max_marks[]"]').val();
				var t_max_marks = parseInt(prc_max_marks) + parseInt(the_max_marks);
	...
	$(document).on('change', '#check_box_sheet_no', function(){
		var checked = $(this).is(':checked');

		if(checked){
			var i=0;
			var first_value = '';
			$('input[name="sheet_no[]"]').each(function(){
				if(i==0){
					first_value = $(this).val();
					return false;
				}
	...
	$('#quickForm').on('change', 'input[name="is_absent[]"]', function(){
		...
		if(checked == true) {
			...
			if(parseInt($(this).closest('.dynamic_wrapper').find('select[name="student_id[]"] option:selected').data('haspractical')) == 0)
			{
				//$(this).closest('.dynamic_wrapper').find('input[name="practical_max_marks[]"]').val('0');
				//$(this).closest('.dynamic_wrapper').find('input[name="practical_max_marks[]"]').attr('readonly', true);
				$(this).closest('.dynamic_wrapper').find('input[name="practical_obt_marks[]"]').val('0');
