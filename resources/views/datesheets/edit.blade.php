          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
	            ...
	            @endif
              <!-- form start -->
              <form id="quickForm" method="post" action="{{ route('datesheets.update') }}">
              	{{ csrf_field() }}
                <input id="hidden_identifier" type="hidden" value="{{ $datesheet->id }}" name="datesheet_id">
                <div class="card-body">
                  <div class="form-group">
                    ...
                    <select class="custom-select rounded-0 select2" id="labelInputSelectSession" name="session_id">
                      ...
                      @foreach($sessions as $session)
                          @if(Request::old('session_id') == '')
                              @if($session->id == $datesheet->session_id)
                   ...
                   <div class="form-group">
                      ...
                      <select class="custom-select rounded-0 select2" id="labelInputSelectSemester" name="semester_id">
                        ...
                        @foreach($semesters as $semester)
                            @if(Request::old('semester_id') == '')
                                @if($semester->id == $datesheet->semester_id)
                                    <option selected value="{{ $semester->id }}">{{ $semester->title }}</option>
                                @else
                                    <option value="{{ $semester->id }}">{{ $semester->title }}</option>
                                @endif
                            @else
                                @if($semester->id == Request::old('semester_id'))
                                    <option selected value="{{ $semester->id }}">{{ $semester->title }}</option>
                                @else
                                    <option value="{{ $semester->id }}">{{ $semester->title }}</option>
                                @endif
                    ...
                    <select class="custom-select rounded-0 select2" id="labelInputSelectSubject" name="subject_id">
                      ...
                      @foreach($subjects as $subject)
                          @if(Request::old('subject_id') == '')
                              @if($subject->id == $datesheet->subject_id)
                    ...
                    <select class="custom-select rounded-0 select2" id="labelInputSelectClass" name="class_id">
                      ...
                      @foreach($standards as $standard)
                          @if(Request::old('class_id') == '')
                              @if($standard->id == $datesheet->class_id)
                   ...
                   <div class="form-group">
                      ...
                      <div class="input-group">
                        ...
                        <input id="datemask" type="text" class="form-control" data-inputmask-alias="datetime" data-inputmask-inputformat="dd-mm-yyyy" data-mask="" inputmode="numeric" name="paper_date" value="{{ Request::old('paper_date') != '' ? Request::old('paper_date') : date('d-m-Y', strtotime($datesheet->paper_date)) }}">
                      </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="input-group date" id="paper_starting_time" data-target-input="nearest">
                      ...
                      <input type="text" class="form-control datetimepicker-input" data-target="#paper_starting_time" name="paper_starting_time" data-inputmask-alias="datetime" data-inputmask-inputformat="HH:MM TT" data-mask="" inputmode="numeric" value="{{ Request::old('paper_starting_time') != '' ? Request::old('paper_starting_time') : date('h:i A', strtotime($datesheet->paper_starting_time)) }}">
                      </div>
                    <!-- /.input group -->
                  </div>
                  <div class="form-group">
                    ...
                    <div class="input-group date" id="paper_ending_time" data-target-input="nearest">
                      ...
                      <input type="text" class="form-control datetimepicker-input" data-target="#paper_ending_time" data-target="#paper_starting_time" name="paper_ending_time" data-inputmask-alias="datetime" data-inputmask-inputformat="HH:MM TT" data-mask="" inputmode="numeric" name="paper_ending_time" value="{{ Request::old('paper_ending_time') != '' ? Request::old('paper_ending_time') : date('h:i A', strtotime($datesheet->paper_ending_time)) }}">
                      </div>
                    <!-- /.input group -->
                  </div>
                <!-- /.card-body -->
