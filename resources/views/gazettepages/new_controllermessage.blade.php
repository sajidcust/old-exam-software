...
<head>
<meta charset="UTF-8">
<title>Gazette Controller Message {{ $session->title }} - {{ $standard->name }}</title>
...
@if($setting->controller_image != '')
<?php $controller_image = ltrim($setting->controller_image, $setting->controller_image[0]); ?>
  <table style="height: 107px; width: 100%; border-collapse: collapse; border-style: none;" border="1">
  <tbody>
  <tr style="height: 39px;">
  <td style="width: 100%; height: 39px; border-style: none; text-align: center;"><img src="{{ $controller_image }}" alt="" width="150"></td>
  </tr>
  </tbody>
  </table>
@endif
