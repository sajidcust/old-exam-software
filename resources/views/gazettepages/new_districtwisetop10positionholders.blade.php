...
<head>
<meta charset="UTF-8">
<title>Gazette Districtwise Top Ten Position Holders Page {{ $session->title }} - {{ $standard->name }}</title>
...
<table width="100%">
  <tr>
      <td style="padding-top:20px;" valign="top" align="center">
        ...
        <h1 style="padding:0px;margin:0px;text-align:center;font-size:15px;font-weight:600;text-decoration:underline;">DISTRICT {{ strtoupper($district->name) }} TOP 10 POSITIONS OF CLASS {{ strtoupper($standard->name) }}</h1>
      </td>
  </tr>
...
<table class="GeneratedTable">
  ...
  <tbody>
    <?php $i=1; ?>
    <?php $gazettes = App\Models\StudentsExam::getToppersByDistrictPages($session->id, $standard->id, $district->id); ?>
    @if(count($gazettes)>0)
