          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
	            ...
	            @endif
              <!-- form start -->
              <form id="quickForm" method="post" action="{{ route('institutions.update') }}">
              	{{ csrf_field() }}
                <input id="hidden_identifier" type="hidden" value="{{ $institution->id }}" name="institution_id">
                <div class="card-body">
                  <div class="form-group">
                    <label for="labelInputInstitutionName">Institution Name<i class="fa fa-star-of-life required-label"></i></label>
                    <input type="text" name="name" class="form-control" id="labelInputInstitutionName" placeholder="Enter institution name" value="{{ Request::old('name')!='' ? Request::old('name'):$institution->name }}">
                   </div>
                   <div class="form-group">
                    ...
                    <select class="custom-select rounded-0" id="labelInputIsDDO" name="is_ddo">
                      @if(Request::old('is_ddo')!='')
                        <option value="1" {{ Request::old('is_ddo') == 1 ? 'selected':'' }}>YES</option>
                        <option value="0" {{ Request::old('is_ddo') == 0 ? 'selected':'' }}>NO</option>
                      @else
                        <option value="1" {{ $institution->is_ddo == 1 ? 'selected' : '' }}>YES</option>
                        <option value="0" {{ $institution->is_ddo == 0 ? 'selected' : '' }}>NO</option>
                      @endif
                    </select>
                   </div>
                   <div class="form-group">
                    <label for="labelInputSelectDDO">Select DDO</label>
                    <select class="custom-select rounded-0 select2" id="labelInputSelectDDO" name="ddo_id" {{ $institution->is_ddo == 1 ? 'disabled':'' }}>
                      <option value="">Select DDO</option> 
                      @foreach($institutions as $institute)
                          @if($institute->id != $institution->id)
                            @if(Request::old('ddo_id') == '')
                                @if($institute->id == $institution->ddo_id)
                                    <option selected value="{{ $institute->id }}">{{ $institute->name }}</option>
                                @else
                                    <option value="{{ $institute->id }}">{{ $institute->name }}</option>
                                @endif
                            @else
                                @if($institute->id == Request::old('ddo_id'))
                                    <option selected value="{{ $institute->id }}">{{ $institute->name }}</option>
                                @else
                                    <option value="{{ $institute->id }}">{{ $institute->name }}</option>
                                @endif
                   ...
                   <div class="form-group">
                    ...
                    <select class="custom-select rounded-0" id="labelInputIsCenter" name="is_center">
                      @if(Request::old('is_center')!='')
                        <option value="1" {{ Request::old('is_center') == 1 ? 'selected':'' }}>YES</option>
                        <option value="0" {{ Request::old('is_center') == 0 ? 'selected':'' }}>NO</option>
                      @else
                        <option value="1" {{ $institution->is_center == 1 ? 'selected' : '' }}>YES</option>
                        <option value="0" {{ $institution->is_center == 0 ? 'selected' : '' }}>NO</option>
                      @endif
                    </select>
                   </div>
                   <div class="form-group">
                    ...
                    <select class="custom-select rounded-0 select2" id="labelInputSelectTehsil" name="tehsil_id">
                      ...
                      @foreach($tehsils as $tehsil)
                          @if(Request::old('tehsil_id') == '')
                              @if($tehsil->id == $institution->tehsil_id)
